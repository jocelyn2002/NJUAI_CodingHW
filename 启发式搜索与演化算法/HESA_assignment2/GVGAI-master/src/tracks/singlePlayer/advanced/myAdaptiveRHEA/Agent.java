...
public class Agent extends AbstractPlayer {
    ...
    private int remaining_actions = 0;
    private Types.ACTIONS action_to_return;
    private boolean reset = true;

    // Parameters
    private int L = 3;
    private int CROSSOVER_TYPE = POINT1_CROSS;
    private int MUTATION = 2;
    ...
    public Agent(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        heuristic = new myHeuristic(stateObs);
...
//            System.out.println("reset!");
            ...
            while (remaining > avgTimeTaken && remaining > BREAK_MS && keepIterating) {
                int[] pas = tunner.getParameters();
                L = pas[0];
                CROSSOVER_TYPE = pas[1];
                MUTATION = pas[2];
                TOURNAMENT_SIZE = pas[3];
                ELITISM = pas[4];
            ...
            // RETURN ACTION
            action_to_return = get_best_action(population);
            remaining_actions = L-1;
            reset = false;
        }
        else {
//            System.out.printf("macro!  %d\n",remaining_actions);
            StateObservation OB = stateObs.copy();
            for (int i=remaining_actions;i>0;i--) {
                OB.advance(action_to_return);
            }
            remaining = timer.remainingTimeMillis();
            while (remaining > avgTimeTaken && remaining > BREAK_MS && keepIterating) {
                int[] pas = tunner.getParameters();
    ...
    private void runIteration(StateObservation stateObs) {
        int r = (int) -evaluate(population[0],heuristic,stateObs);
        ...
        r += (int)nextPop[0].value;
        tunner.UpdateParameters(r);
