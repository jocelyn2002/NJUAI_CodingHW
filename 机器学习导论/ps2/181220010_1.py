...
beta = np.zeros([11,1])
for time in range(nt_times):
    A = B = 0
    for i in range(len(x)):
        xx = x[i].reshape([11,1])
        p1 = (np.e ** (np.dot(beta.T,xx)) )/(1+np.e ** (np.dot(beta.T,xx)))
        A -= xx*(y[i]-p1)
        B += np.matmul(xx,xx.T)*p1*(1-p1)
    beta -= np.matmul(np.linalg.pinv(B),A)
# print(beta)
...
y_predict.to_csv('181220010_1.csv',header=0,index=0)