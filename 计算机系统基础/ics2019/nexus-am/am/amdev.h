...
_AM_DEVREG(TIMER,  UPTIME, 1, uint32_t hi, lo);                           // 系统启动毫秒数
_AM_DEVREG(TIMER,  DATE,   2, int year, month, day, hour, minute, second);// 日期
_AM_DEVREG(VIDEO,  INFO,   1, int width, height);
_AM_DEVREG(VIDEO,  FBCTL,  2, int x, y; uint32_t *pixels; int w, h, sync);// (x,y)坐标处w*h矩形绘制pixels数组中的内容
