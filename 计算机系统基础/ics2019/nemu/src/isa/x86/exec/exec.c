...
} // 0时按is_operand_size16来，否则指定为width（字节），decinfo三数合一
...
static OpcodeEntry opcode_table [512] = {
  ...
  /* 0x68 */	IDEX(push_SI,push), IDEX(I_E2G,imul3), IDEXW(I,push,1), IDEX(I_E2G, imul3),
...
void isa_exec(vaddr_t *pc) {
  uint32_t opcode = instr_fetch(pc, 1); // 读一个字节
  decinfo.opcode = opcode;
  set_width(opcode_table[opcode].width);
  idex(pc, &opcode_table[opcode]);
} // 读opcode，译码，设置宽度，执行