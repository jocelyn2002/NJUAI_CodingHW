...
//	 * @param value The value to set.
	...
	private float block2() {
		float value =0;
		for (byte i=2;i<=5;i++) {
			if (getSpotOnLine(hBoard[2], i)==3)
				value+=1;
			else if (getSpotOnLine(hBoard[2], i)==2)
				value-=1;
		}
		for (byte i=2;i<=5;i++) {
			if (getSpotOnLine(hBoard[5], i)==3)
				value+=1;
			else if (getSpotOnLine(hBoard[5], i)==2)
				value-=1;
		}
		for (byte i=3;i<=4;i++) {
			if (getSpotOnLine(vBoard[2], i)==3)
				value+=1;
			else if (getSpotOnLine(vBoard[2], i)==2)
				value-=1;
		}
		for (byte i=3;i<=4;i++) {
			if (getSpotOnLine(vBoard[5], i)==3)
				value+=1;
			else if (getSpotOnLine(vBoard[5], i)==2)
				value-=1;
		}
		System.out.printf("block2: %f\n",value);
		return value;
	}
	private float block4(){
		float value =0;
		for (byte i=0;i<=7;i++) {
			if (getSpotOnLine(hBoard[0], i)==3)
				value+=1;
			else if (getSpotOnLine(hBoard[0], i)==2)
				value-=1;
		}
		for (byte i=0;i<=7;i++) {
			if (getSpotOnLine(hBoard[7], i)==3)
				value+=1;
			else if (getSpotOnLine(hBoard[7], i)==2)
				value-=1;
		}
		for (byte i=1;i<=6;i++) {
			if (getSpotOnLine(vBoard[0], i)==3)
				value+=1;
			else if (getSpotOnLine(vBoard[0], i)==2)
				value-=1;
		}
		for (byte i=1;i<=6;i++) {
			if (getSpotOnLine(vBoard[7], i) == 3)
				value += 1;
			else if (getSpotOnLine(vBoard[7], i) == 2)
				value -= 1;
		}
//		System.out.printf("7,7: %d\n",getSpotOnLine(vBoard[7], (byte)7));
		System.out.printf("block4: %f\n",value);
		return value;
	}
	private float semicorner(){
		float value =0;

		if (getSpotOnLine(hBoard[1], (byte)1)==3)
			value+=1;
		else if (getSpotOnLine(hBoard[1], (byte)1)==2)
			value-=1;

		if (getSpotOnLine(hBoard[1], (byte)6)==3)
			value+=1;
		else if (getSpotOnLine(hBoard[1], (byte)6)==2)
			value-=1;

		if (getSpotOnLine(hBoard[6], (byte)6)==3)
			value+=1;
		else if (getSpotOnLine(hBoard[6], (byte)6)==2)
			value-=1;

		if (getSpotOnLine(hBoard[6], (byte)1)==3)
			value+=1;
		else if (getSpotOnLine(hBoard[6], (byte)1)==2)
			value-=1;

		System.out.printf("semicorner: %f\n",value);
		return value;
	}
	...
	public float heuristic() {
		...
		float total_value = this.pieceDifferential() +
