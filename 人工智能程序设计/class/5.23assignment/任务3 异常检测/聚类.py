...
house.replace(('republican','democrat'),(0,1),inplace=True)
pca = PCA(n_components=2)
pca.fit(house)
x = pd.DataFrame(pca.transform(house)).astype('float')
# 进行离群分析
l2 = 0.1
x2 = [[], []]
for i in range(0, len(x.index)):
    ...
    for j in range(i+1,len(x.index)):
        if (x.iloc[i,0]-x.iloc[j,0])**2+(x.iloc[i,1]-x.iloc[j,1])**2 <= l2:
            s=1
            break
    if s == 0:
        x2[0].append(x.iloc[i, 0])
        x2[1].append(x.iloc[i, 1])
trace1=go.Scatter(
...
plotly.offline.plot([trace1,trace2], filename='降维分析')
